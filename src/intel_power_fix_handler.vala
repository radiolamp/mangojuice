    ...
    try {
        ...
        try {
            ...
            Process.spawn_command_line_sync (
                "systemctl is-active --quiet powercap-permissions.service",
        ...
        var dialog = new Adw.AlertDialog (
            _("Warning"),
            _("You are changing the rights to intel energy_uj, which could potentially lead to security issues.")
        );
        ...
        if (response == "temporary") {
            Process.spawn_command_line_sync (
                "pkexec chmod " + new_mode + " /sys/class/powercap/intel-rapl\\:0/energy_uj"
            );
            ...
            if (service_exists) {
                ...
                Process.spawn_command_line_sync (
                    "pkexec sh -c 'echo \"" + service_content + "\" " +
                    "> /etc/systemd/system/powercap-permissions.service && " +
                    "systemctl daemon-reload && " +
                ...
                var reboot_dialog = new Adw.AlertDialog (
                    _("Warning"),
                    _("For the permanent changes to take full effect, " +
                      "a reboot is recommended. Would you like to reboot now?")
                );
    ...
    Idle.add (() => {
        if (has_permissions) {
            ...
            button.set_tooltip_text (
                is_service_active
                ? _("Permissions set permanently (systemd service active)")
            ...
            button.set_tooltip_text (
                is_service_active
                ? _("Service active but permissions not set")
